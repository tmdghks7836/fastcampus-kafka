...
public class KafkaManger {
    ...
    public KafkaManger(KafkaAdmin kafkaAdmin) {
    ...
    public void changeConfig() throws ExecutionException, InterruptedException {
        ...
        ops.put(resource, List.of(new AlterConfigOp(new ConfigEntry(TopicConfig.RETENTION_MS_CONFIG, null), AlterConfigOp.OpType.SET)));
    ...
    public void deleteRecords() throws ExecutionException, InterruptedException {
        ...
        for (Map.Entry<TopicPartition, KafkaFuture<DeletedRecords>> entry : entries) {
            ...
            System.out.println("entry.getKey().topic() = " + entry.getKey().topic());
            System.out.println("entry.getKey().partition() = " + entry.getKey().partition());
            System.out.println("entry.getValue().get() = " + entry.getValue().get().lowWatermark());
        }
    }
    ...
    public void findAllOffset() throws ExecutionException, InterruptedException {
        ...
        for (TopicPartition tp : target.keySet()) {
            System.out.println("tp.topic() = " + tp.topic());
            System.out.println("tp.partition() = " + tp.partition());
            System.out.println("result.partitionResult(tp).get() = " + result.partitionResult(tp).get());
        }
    }
}
